    ...
    public void handleClick(@Nullable Expandable expandable) {
        if (!mCanSwitch) {
            ...
            SysUIToast.makeText(mContext,
                    mContext.getString(R.string.quick_settings_data_switch_toast_0),
                    Toast.LENGTH_LONG).show();
        } else if (mSimCount == 1) {
            Log.d(TAG, "handleClick:only one sim card");
            SysUIToast.makeText(mContext,
                    mContext.getString(R.string.quick_settings_data_switch_toast_1),
                    Toast.LENGTH_LONG).show();
        } else {
    ...
    protected void handleUpdateState(BooleanState state, Object arg) {
        ...
        switch (mSimCount) {
            case 1:
                state.icon = ResourceIcon.get(activeSIMZero
                        ? R.drawable.ic_quick_settings_data_switch_1
                        : R.drawable.ic_quick_settings_data_switch_2);
                state.secondaryLabel = mContext.getString(
                        activeSIMZero ? R.string.quick_settings_data_switch_text_1
                                      : R.string.quick_settings_data_switch_text_2);
                state.value = false;
                break;
            case 2:
                state.icon = ResourceIcon.get(activeSIMZero
                ...
                break;
            default:
                state.icon = ResourceIcon.get(R.drawable.ic_quick_settings_data_switch_1);
                state.value = false;
                break;
        }
        if (mSimCount < 2) {
        ...
        state.contentDescription =
                mContext.getString(activeSIMZero
                        ? R.string.quick_settings_data_switch_changed_1
                        : R.string.quick_settings_data_switch_changed_2);
